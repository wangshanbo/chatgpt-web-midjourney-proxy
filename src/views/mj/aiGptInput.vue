...
const handleSubmit = () => {
  ...
  // 获取env中的API_KEY
  const apiKey = gptServerStore.myData.OPENAI_API_KEY || localStorage.getItem("apiKey");

  // if (!apiKey && gptConfigStore.myData.model != "gpt-4o-mini-2024-07-18") {
  //   ms.error("请选择gpt-4o-mini-2024-07-18模型或者设置正确的API_KEY");
  //   return false;
  // }
    ...
    <div class="w-full relative">
      <div class="flex items-base justify-start pb-1 flex-wrap-reverse" v-if="st.fileBase64.length > 0">
        <div class="w-[60px] h-[60px] rounded-sm bg-slate-50 mr-1 mt-1 text-red-300 relative group"
          v-for="(v, ii) in st.fileBase64" :key="ii">
          ...
          <template #prefix>
            <div class="relative; w-[22px]">
              <n-tooltip trigger="hover">
                <template #trigger>
                  <SvgIcon icon="line-md:uploading-loop" class="absolute bottom-[10px] left-[8px] cursor-pointer"
                    v-if="st.isLoad == 1"></SvgIcon>
                  <SvgIcon icon="ri:upload-2-fill" class="absolute bottom-[10px] left-[8px] cursor-pointer"
                    @click="fsRef.click()" v-else></SvgIcon>
                </template>
                <div v-if="canVisionModel(gptConfigStore.myData.model)" v-html="$t('mj.upPdf')"></div>

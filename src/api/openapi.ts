...
export const getSystemMessage = (uuid?: number) => {
    ...
    let model = gptConfigStore.myData.model ? gptConfigStore.myData.model : "gpt-4o-mini-2024-07-18";
...
export const subModel = async (opt: subModelType) => {
    //
    const apiKey = localStorage.getItem('apiKey');
    let model = opt.model ?? (gptConfigStore.myData.model ? gptConfigStore.myData.model : "gpt-4o-mini-2024-07-18");
...
export const openaiSetting = (q: any) => {
    ...
    if (q.settings) {
        ...
        try {
            ...
            gptServerStore.myData.OPENAI_API_BASE_URL = url;
...
export const countTokens = async (dataSources: Chat.Chat[], input: string, uuid: number) => {
    ...
    rz.history = msg.length == 0 ? 0 : encodeChat(msg, model.indexOf('gpt-4') > -1 ? 'gpt-4' : 'gpt-4o-mini-2024-07-18').length
    //
...
const getModelMax = (model: string) => {
    ...
    if (model.indexOf('8k') > -1) {
        return 8;
    } else if (model.indexOf('16k') > -1 || model == 'gpt-4o-mini-2024-07-18-1106' || model == 'gpt-4o-mini-2024-07-18-0125') {
        return 16;
